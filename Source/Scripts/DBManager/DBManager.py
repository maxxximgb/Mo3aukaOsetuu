...
class DBManager:
    ...
    def save_all(self):
        ...
        self.cursor.execute('''
            INSERT INTO GameState (score, current_level_id)
            VALUES (?, ?)
        ''', (game_state.score, game_state.currentlvl.id if game_state.currentlvl else None))
    ...
    def load_all(self):
        self.cursor.execute('SELECT score, current_level_id FROM GameState ORDER BY id DESC LIMIT 1')
